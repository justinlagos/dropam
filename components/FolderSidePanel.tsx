/**
 * Side panel for a folder on the pod canvas. Shows folder name, briefs inside, and actions.
...
import { Folder, Brief, Brand } from '../types';
...
export const FolderSidePanel: React.FC<FolderSidePanelProps> = ({
  folder,
  briefsInFolder,
  brands,
  onClose,
  ...
  const { updateFolderName, moveBriefToFolder, deleteFolder } = useActions();
  ...
  const handleSaveName = () => {
    ...
    if (trimmed && trimmed !== folder.name) {
      updateFolderName(folder.id, trimmed);
    }
      ...
      <div className="w-[400px] p-6 h-full flex flex-col overflow-y-auto no-scrollbar">
        ...
        <div>
          ...
          {briefsInFolder.length === 0 ? (
            <p className="text-sm text-gray-400 py-4">Drag briefs from the canvas into this folder, or use the context menu &quot;Move briefs here&quot;.</p>
          ) : (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <button
                        ...
                        className="text-sm font-medium text-[#111111] truncate block text-left hover:underline w-full"
                      >
                        {brief.title}
                      </button>
                      {brand && (
                        <span className="text-[10px] uppercase tracking-wider text-gray-400">{brand.name}</span>
                      )}
                    </div>
                    <button
                      type="button"
                      onClick={() => moveBriefToFolder(brief.id, undefined)}
