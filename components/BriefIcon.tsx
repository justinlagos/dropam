...
export const BriefIcon: React.FC<BriefIconProps> = ({
  ...
  // Reset motion values when brief position changes externally
  useEffect(() => {
    x.set(0);
    y.set(0);
  ...
  return (
    <>
      <motion.div
        ...
        style={{
          ...style,
          x: canDrag ? x : undefined,
          y: canDrag ? y : undefined,
        }}
        ...
        animate={isTarget ? { scale: 1.2 } : isDragging ? { scale: 1.05, zIndex: 100 } : { scale: 1 }}
        className={`${canDrag ? 'draggable-item' : ''} w-[120px] h-[140px] flex flex-col items-center justify-start pt-2 cursor-pointer select-none rounded-2xl transition-colors duration-150 ease-out
